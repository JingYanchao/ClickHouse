...
static DataTypePtr createSortedStringKVType()
{
    ...
    /// Reuse the Tuple's own serialization to extract ElementSerializations,
    /// keeping it in sync with DataTypeTuple::doGetSerialization.
    auto tuple_ser = std::dynamic_pointer_cast<const SerializationTuple>(type->getDefaultSerialization());
    auto elems = tuple_ser->getElementsSerializations();
    ...
    type->setCustomization(std::make_unique<DataTypeCustomDesc>(
        ...
        std::make_shared<SerializationSortedStringKV<V>>(elems, true)));
