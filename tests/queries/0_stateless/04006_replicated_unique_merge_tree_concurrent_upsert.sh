...
$CLICKHOUSE_CLIENT --query "CREATE TABLE test_concurrent_r1 (x UInt32, y UInt32, PROJECTION __unique_index INDEX x TYPE unique) ENGINE = ReplicatedUniqueMergeTree('/clickhouse/tables/${CLICKHOUSE_DATABASE}/test_04006/concurrent_upsert', '1') ORDER BY x"
$CLICKHOUSE_CLIENT --query "CREATE TABLE test_concurrent_r2 (x UInt32, y UInt32, PROJECTION __unique_index INDEX x TYPE unique) ENGINE = ReplicatedUniqueMergeTree('/clickhouse/tables/${CLICKHOUSE_DATABASE}/test_04006/concurrent_upsert', '2') ORDER BY x"
